...
const ManageFSEC = () => {
  ...
  let approvedTransactions = filteredApprovedTransactions.map((txn) => {
    if (txn.txnName === "FSEC Application" && txn.pe === "Approved" ) {
      return (
        <tr key={txn._id}>
  ...
  return (
    <>
      <h1 className="text-center">Manage FSEC</h1>
      <h2>Approved</h2>
      <Row md={4}>
        <Col>
          <Form.Control placeholder="Search Approved FSEC" onChange={(e) => setApprovedSearchText(e.target.value)}/>
        </Col>
      </Row>
      ...
      <Row md={4}>
        <Col>
          <Form.Control placeholder="Search Disapproved FSEC" onChange={(e) => setDisApprovedSearchText(e.target.value)}/>
        </Col>
      </Row>
