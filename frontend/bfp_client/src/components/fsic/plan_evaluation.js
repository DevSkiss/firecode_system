...
const PlanEvaluation = () => {
  ...
  const showTable = filteredPlan.map((plan) => {
    if (
      (plan.pe === "Pending" &&
        plan.txnName === "FSIC Application for Business" &&
        plan.status === "Paid") ||
      (plan.pe === "Pending" &&
        plan.status === "Paid" &&
        plan.txnName === "FSIC Application for Occupancy")
    ) {
    ...
    <>
      ...
      >
        ...
        <Modal.Body>
          ...
          <p> As Plan Evaluator Do you Approve or Disapprove</p>
          <Form.Group>
            <Form.Label>
