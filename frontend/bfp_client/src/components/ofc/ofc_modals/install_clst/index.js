...
const InstallClst = (props) => {
  const [clst, setClst] = useState(0);
  const [resultClst, setResultClst] = useState(0);
  const handleClose = props.onHide;
  const { setPayments, handleClose2 } = useContext(OtherContext);

  useEffect(() => {
    setResultClst(clst * 0.001);
  }, [clst]);
  ...
  const submitClst = () => {
    if (resultClst != 0) {
      setPayments((payments) => [
        ...payments,
        {
          paymentName: "Installation of Combustible Liquid Storage Tanks",
          firecode: "628-BFP-09",
          amount: Math.round(resultClst),
        },
      ]);
      handleClose2();
      ...
      >
          ...
          <Row className="justify-content-center">
            <Col md={6}>
              <Form.Control
                ...
                onChange={(e) => setClst(e.target.value)}
              />
            </Col>
          </Row>
        </Modal.Body>
        <Modal.Footer>
          <div className={styles.footer}>
            <p className={styles.textFooter}>P {Math.round(resultClst)}</p>
            <Button
              bsPrefix
              className={styles.button}
              onClick={(e) => submitClst()}
            >
