...
export default function Collection() {
  ...
  let filteredTxn = txnData.filter((txn) => {
    if (searchText === "") {
      return txn;
    } else {
      return (
  ...
  const transactions = filteredTxn.map((txn) => {
    if (txn.status === "Unpaid" && txn.txnName === "Other Fee/Clearances") {
      return (
        <tr key={txn._id}>
    ...
    <>
      ...
      >
        ...
        <Modal.Body>
          ...
          <p>
            <strong>
              Total Amount Of: <small className="text-style">₱ {amount}</small>
            </strong>
          </p>
          <Button
